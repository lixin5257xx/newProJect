...
function init() {
    ...
    function onScroll(event) {
        if (!flag) {
            var closeToBottom = ($(window).scrollTop() + $(window).height() > $(document).height() - 30);
            if (closeToBottom) {
                sendData.pageNo++;
                if (categoryTypeId != 5) {
                    update();
...
//衣服小类选择
    function clothesPartSelect() {
        var codeAry = [];
        var filterAry = [];
        $(".type-part >.type-part-item").each(function (index,item) {
        ...
        $(".type-part-li li").on('click', function () {
            ...
            if(!$(this).hasClass('selected')){
                ...
                //filterAry 中添加
                if(filterAry.indexOf(curId) ==-1){
                    curId = "'" + curId + "'";
                    filterAry.push(curId);
                }
            }else {
                $(this).removeClass('selected');
                var selectedLen = $(this).parent().children('.selected').length;
                if(selectedLen == 0){
                    var curIndex = codeAry.indexOf(curCode);
                    codeAry.splice(curIndex,1);
                    if(codeAry.length == 0) sendData.code = "none";
                }
                var filterIndex = filterAry.indexOf(curId);
                filterAry.splice(filterIndex,1);
            }
...
// 点赞、收藏功能
    ...
    function getInvidateCode() {
        $(".get-invidate-code").on("click", function () {
            ...
            countDown();
            //获取短信验证码
            getInvidateCodeInfo(regionData, false, function (result) {
                reInvidateCode = result.data.vCode;
                console.log('短信验证码', reInvidateCode);
            })
        });
    }
...
function update(isRefresh) {
    ...
    if (isRefresh) {    // 页面加载
        ...
        getAccountListInfo(sendData, true, function (result) {
            ...
            $.each(accountListData.data.list, function (index, item) {
                $.ajax({
                    url: item.icon + "?imageInfo",
                    type: "get",
                    dataType: "json",
                    async: false,
                    success: function (data) {
                        item.imgW = data.width/2 + "px";
                        item.imgH = data.height/2 + "px";
                    },
                    error: function (err) {
                        console.log(err);
                    }
...
function bindClothData(result) {
    ...
    for (; i < length; i++) {
        ...
        html += '<a href="javascript:;"><img src="' + item.icon + '" style="height: ' + item.imgH + '"></a>';
        html += '</div>';
        ...
        if (item.isRent) {
            if (item.isRent == 0) {
                ...
                html += '<p><i class="tc clothes-label border-radius-5 mr-5">可租</i>' + item.title + '</p>';
            }
        } else {
            html += '<p>' + item.title + '</p>';
        }
...
function bindWeddingHouse(result) {
    ...
    for (; i < length; i++) {
        ...
        weddingStr += '<dd>';
        weddingStr += '<div class="describe line">';
